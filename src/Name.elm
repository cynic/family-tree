module Name exposing (Model, Msg(..), newName, update, view)
                        ...
                        (\i ->
                            \nickname ->
                                if i == idx then
                                    let
                                        value =
                                            Nickname.update nickMsg nickname
                                    in
                                    if Nickname.isDeleted value then
                                        Nothing
...
view model =
    div [ class "all-names" ]
        ...
        , elideIf (List.isEmpty model.nicknames)
            (div [ class "nicknames" ])
            ((model.nicknames
                |> List.indexedMap
                    (\idx ->
...
newName : Model
newName =
    { name = PrimaryName.newName, nicknames = [] }
